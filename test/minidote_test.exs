defmodule DistributedDataStoreTest do
  use ExUnit.Case
  doctest DistributedDataStore

  test "greets the world" do
    # ToDo: This fails, so feel free to improve the tests :)
    assert DistributedDataStore.ping() == :world
  end
  ...
  test "simple counter replication" do
    ...
    {:ok, vc} = :rpc.call(dc1, :"Elixir.DistributedDataStore", :modify_data_items, [[{{"key", :counter_pn_ob, "simple counter replication"}, :increment, 42}], :ignore])
    # reading on the same replica returns 42
    {:ok, [{{"key", :counter_pn_ob, "simple counter replication"}, 42}], _vc2} = :rpc.call(dc1, :"Elixir.DistributedDataStore", :retrieve_data_items, [[{"key", :counter_pn_ob, "simple counter replication"}], vc])
    # reading on the other replica returns 42
    {:ok, [{{"key", :counter_pn_ob, "simple counter replication"}, 42}], _vc2} = :rpc.call(dc2, :"Elixir.DistributedDataStore", :retrieve_data_items, [[{"key", :counter_pn_ob, "simple counter replication"}], vc])
